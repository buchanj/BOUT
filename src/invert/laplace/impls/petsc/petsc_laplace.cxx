...
LaplacePetsc::LaplacePetsc(Options *opt) : Laplacian(opt) {
  ...
  // Calculate Global Size in X dimension
  ...
  // Set size of Matrix on each processor localN x size on each processor
  MatCreate( comm, &MatA );                                
  MatSetSizes( MatA, localN, size, size, size );                  
  MatSetFromOptions(MatA);                                       
  MatMPIAIJSetPreallocation( MatA,5, PETSC_NULL, 5, PETSC_NULL ); 
  MatSetUp(MatA); 
...
const FieldPerp LaplacePetsc::solve(const FieldPerp &b, const FieldPerp &x0) {
  ...
  // Determine which row/columnss of the matrix are locally owned
  int Istart, Iend;    // Istart is the first row owned, Iend is 1 greater than the last row.
  ...
  if(mesh->firstX()) {
    for(int x=0; x < mesh->xstart; x++)
      for(int z=0;z < mesh->ngz-1; z++) {
        ...
        int iup   = i+meshz; // location of node at same z in next row up in x
	...
	if(flags & INVERT_AC_IN_GRAD) 
	  MatSetValues(MatA,1,&i,1,&iup,&val,INSERT_VALUES);  
  ...
  if(mesh->lastX()) {
    for(int x=mesh->xend+1; x < mesh->ngx; x++)
      for(int z=0;z < mesh->ngz-1; z++) {
        int idown   = i-meshz; // location of node at same z in next row down in x
	...
	if(flags & INVERT_AC_OUT_GRAD) 
	  MatSetValues(MatA,1,&i,1,&idown,&val,INSERT_VALUES); 
  ...
  // Solve the system using Magik. - FIXME add functionality to change solver type from options.
  KSPSolve( ksp, bs, xs );               
...
void LaplacePetsc::Coeffs( int x, int y, int z, BoutReal &coef1, BoutReal &coef2, BoutReal &coef3, BoutReal &coef4, BoutReal &coef5 )
{
  ...
  if(nonuniform) {
    ...
    if((x != 0) && (x != (mesh->ngx-1))) {
      ...
      coef4 = coef4 - 0.5*((mesh->dx[x+1][y] - mesh->dx[x-1][y])/SQ(mesh->dx[x][y]))*coef1; // BOUT-06 term
    }
  }
